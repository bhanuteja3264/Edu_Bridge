const createProjectForumSlice = (set) => ({
    forumProjects: [
        {
            id: 1,
    ...
    updateForumProject: (projectId, updates) => set((state) => ({
        forumProjects: state.forumProjects.map(project =>
            project.id === projectId ? { ...project, ...updates } : project
        )
    })),

    removeForumProject: (projectId) => set((state) => ({
        forumProjects: state.forumProjects.filter(project => project.id !== projectId)
    }))
});

export default createProjectForumSlice; 